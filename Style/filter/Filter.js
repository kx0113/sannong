...
(function ($) {
    ...
    $.fn.comboboxfilter.methods = {
        ...
        setData: function (target, options, data, targetContain) {
            ...
            $.each(data, function (i, item) {
                var listnode = $(' <li></li>');
                var clicka = $('<a class="filter-tag" href="javascript:;" data-value="' + item[options.idField] + '" data-text="' + item[options.textField] + '">' + item[options.textField] + '-'+item[options.deptField]+'<i></i></a>').data('data', item);
    ...
    $.fn.comboboxfilter.defaults = {
        url: '',	
        idField: 'uid',
        textField: 'real_name',//姓名
        deptField: 'department',//部门
        scope: 'FilterQuery',
        text: '',
        multiple: false,
	data:[],
        inputName: '',
        unlimit: false,//是否显示不限，默认显示
        unlimitText:'不限',
        param:{},

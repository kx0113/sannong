                    ...
                    <div class='xzen xzen1'>
                        ...
                        <ul style='padding-top:40px; padding-bottom:50px;'>
                            <form method="post" enctype="multipart/form-data">
                                <!--<li>-->
                                    ...
                                    <!--<select name="menu_id">-->
                                        <!--<?php if(is_array($menu)): $i = 0; $__LIST__ = $menu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>-->
                                            <!--<option value="<?php echo ($vo["mid"]); ?>" <?php if($vo[mid] == $info[menu_id]): ?>selected<?php endif; ?> ><?php echo ($vo["mname"]); ?></option>-->
                                            <!--<?php if(is_array($vo["_child"])): $i = 0; $__LIST__ = $vo["_child"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$child): $mod = ($i % 2 );++$i;?>-->
                                                <!--<option value="<?php echo ($child["mid"]); ?>" <?php if($child[mid] == $info[menu_id]): ?>selected<?php endif; ?> >&emsp;&emsp;<?php echo ($child["mname"]); ?></option>-->
                                ...
                                <!--<li>-->
                                    <!--<span>信息标题</span>-->
                                    <!--<input type='' name="title" value="<?php echo ($info["title"]); ?>">-->
                                <!--</li>-->
                                <li>
                                    <span>应用类型：</span>
                                    <?php if($type == 1): ?><input name="type" readonly value="易管理"/>
                                        <?php elseif($type == 2): ?>
                                        <input name="type" readonly value="易家家"/><?php endif; ?>
                                    <!--<select name="type" id="type">-->
                                        <!--<?php if($_GET['id']): ?>-->
                                            <!--<?php if($info["type"] == 1): ?>-->
                                                    <!--<option  value="1">易管理</option>-->
                                                <!--<?php elseif($info["type"] == 2): ?>-->
                                                    <!--<option  value="2">易家家</option>-->
                                            <!--<?php endif; ?>-->
                                            <!--<?php else: ?>-->
                                            <!--<option value="0">请选择应用</option>-->
                                            <!--<option <?php if($info["type"] == 1): ?>selected="selected"<?php endif; ?> value="1">易管理</option>-->
                                            <!--<option <?php if($info["type"] == 2): ?>selected="selected"<?php endif; ?> value="2">易家家</option>-->
                                        <!--<?php endif; ?>-->
                                    <!--</select>-->
                                </li>
                                <li>
                                    <span>展示图片：</span><br/>
                                    <?php if($info["picture"] != ''): ?><img src="/Uploads/<?php echo ($info["picture"]); ?>" style="margin-left: 87px;width: 300px;" /><br/><br/><?php endif; ?>
                                ...
                                <!--</li>-->
                                <li class='xh-an'>
                                    <button type="submit" style="margin-right: 100px;">保存</button>
                                    <button type="button" onclick="javascript:history.go(-1);" class="czhi">取消</button>
                                </li>
                            </form>
                        </ul>
                    </div>
...
<script type="text/javascript">
    var cont = '<?php echo ($info["content"]); ?>';

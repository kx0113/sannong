            ...
            <div class="col-lg-12">
                <section class="box ">
                    <header class="panel_header">
                        <h2 class="title pull-left">菜单列表</h2>
                        <div class="actions panel_actions pull-right">
                            <div class='pl cle' style='margin-bottom:10px;'>
                                <!--<a href="" class="da da1"><i class="fa fa-upload"></i>导入</a>-->
                                <a href="<?php echo U('addMenu');?>" class="da da2"><i class="fa fa-plus"></i>新增</a>
                                <!--<a href="javascript:;" class="da da3"><i class="fa fa-trash"></i>删除</a>-->
                                <?php if($dept != ''): ?><label>
                                        <form method="get">
                                            <select name="dept_id">
                                                <?php if(is_array($dept)): $i = 0; $__LIST__ = $dept;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["did"]); ?>" <?php if($cz == $vo['did']): ?>selected<?php endif; ?>><?php echo ($vo["dname"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                                            </select>
                        ...
                        <div class="row">
                            <div class="col-xs-12">
                                <table id="example-1" class="table table-striped dt-responsive display table1">
                                    ...
                                    <tbody>
                                    <?php if(is_array($menu)): $i = 0; $__LIST__ = $menu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr data-in='0'>
                                            <td style="text-align: center;"><?php echo ($vo["mname"]); ?></td>
                                            <td>一级菜单</td>
                                            <td><?php echo ($vo["morder"]); ?></td>
                                            <td>
                                                <?php if($vo['type'] != 1): ?><a onclick="return confirm('确定删除此菜单及其子菜单？')" href="/Manage/Dept/delMenu/mid/<?php echo ($vo["mid"]); ?>" class='cz'><i class='fa fa-trash' data-nub='0'></i></a>
                                                    <a href="<?php echo U('Manage/Dept/setMenu',array(mid=>$vo[mid]));?>" class='cz'><i class='fa fa-pencil'></i></a><?php endif; ?>
                                            </td>
                                        </tr>
                                        <?php if(is_array($vo["_child"])): $i = 0; $__LIST__ = $vo["_child"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$child): $mod = ($i % 2 );++$i;?><tr data-in='0'>
                                                <!--<td><span class='dx-zi'></span></td>-->
                                                <td></td>
                                                <td><span style=""><?php echo ($child["mname"]); ?></td>
                                                <td><?php echo ($child["morder"]); ?></td>
                                                <td>
                                                    <a onclick="return confirm('确定删除此菜单？')" href="/Manage/Dept/delMenu/mid/<?php echo ($child["mid"]); ?>" class='cz'><i class='fa fa-trash' data-nub='0'></i></a>
                                                    <a href="<?php echo U('Manage/Dept/setMenu',array(mid=>$child[mid]));?>" class='cz'><i class='fa fa-pencil'></i></a>
                                                </td>

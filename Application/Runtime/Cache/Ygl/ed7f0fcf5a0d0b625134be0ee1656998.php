    ...
    <body class='bc1'>
            ...
            <div id="leftTabBox" class="tabBox vertical-view">
                <div class='hd maple-tab clearFix'>
                    <ul class='page' l id="pagenavi">
                        <li <?php if($cz == 1): ?>class='active'<?php endif; ?>  data-li='1'><a href='/Ygl/Public/jgqs2?dept_id=1'>畜牧品</a></li>
                        <li <?php if($cz == 2): ?>class='active'<?php endif; ?> data-li='2'><a href="/Ygl/Public/jgqs2?dept_id=2">粮油</a></li>
                        <li <?php if($cz == 3): ?>class='active'<?php endif; ?> data-li='3'><a href="/Ygl/Public/jgqs2?dept_id=3">果品</a></li>
                        <li <?php if($cz == 4): ?>class='active'<?php endif; ?> data-li='3'><a href="/Ygl/Public/jgqs2?dept_id=4">蔬菜</a></li>
                    </ul>
                </div>
        ...
        <script type="text/javascript">
            ...
            option = {
                ...
                legend: {
                    data:[<?php if(is_array($infos)): $i = 0; $__LIST__ = $infos;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>'<?php echo ($vo["cate_name"]); ?>',<?php endforeach; endif; else: echo "" ;endif; ?>] //
            },
            grid: {
                left: '2%',
                        right: '6%',
            ...
            xAxis: {
                type: 'category',
                        boundaryGap: false,
                        data: [<?php if(is_array($infos)): $i = 0; $__LIST__ = array_slice($infos,0,1,true);if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if(is_array($vo["adddate"])): foreach($vo["adddate"] as $key=>$vv): ?>'<?php echo ($vv); ?>',<?php endforeach; endif; endforeach; endif; else: echo "" ;endif; ?>]
            },
            yAxis: {
                type: 'value'
            },
            ...
            <?php if(is_array($infos)): $k = 0; $__LIST__ = $infos;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?>{
                name:'<?php echo ($vo["cate_name"]); ?>',
                type:'line',
                data:[<?php if(is_array($vo["child"])): foreach($vo["child"] as $key=>$vv): echo ($vv["price"]); ?>,<?php endforeach; endif; ?>]
